...
server_block = False
set_client_cmd = b''
server_data_recv_flage = False
authkey = ''
conn_flage = False

pathlog ='/WIZSTICK_LOG/SSH/'+strftime("%Y", localtime())+'/'+strftime("%m%d", localtime())

#mypool = customDB.pool.QueuePool(customDB.getconn, max_overflow=3, pool_size=2, recycle=10)


def split_logfile_save(LOGFILE, LOG_START_TIME):
    customDB.mysql_insert_bin_B((LOG_START_TIME, strftime("%Y-%m-%d %H:%M:%S", localtime()), server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, 'BASE64', SESSION_ID), LOGFILE)
    '''
    data = ((LOG_START_TIME, strftime("%Y-%m-%d %H:%M:%S", localtime()), server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, 'BASE64', SESSION_ID), LOGFILE)
    while True:
        if r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'mysql_insert_bin_B', 'Sub_type':'','data':cjson.encode(data)})) == 1:
            break
        time.sleep(0.2)
    '''
...
def terminate(origin):
    '''
    Shutdown the twisted reactor.
    '''
    #print "Shutdown the twisted reactor.", [origin]
    global SESSION_ID, tmp_auth, LOG_START_TIME, LOGFILE, f_terminate
    #if origin == 'client' or origin == 'server' :
    if f_terminate == False:
        f_terminate = True
        ...
        r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'mysql_log_insert', 'Sub_type':'','data':(strftime("%Y-%m-%d %H:%M:%S", localtime()), server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, 102, 0, 'LogOut', 'TEXT')}))
        split_logfile_save(LOGFILE, LOG_START_TIME)
        #customDB.mysql_insert_bin(mypool, (LOG_START_TIME, strftime("%Y-%m-%d %H:%M:%S", localtime()), server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username), LOGFILE)
    ...
    '''
    while True:
        if r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'close', 'Sub_type':'','data':''})) == 1:
            break
        time.sleep(0.5)
    '''
    r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'close', 'Sub_type':'','data':''})) 
...
def ssh_proxy_option_parser(port, localport):
    ...
    parser.add_option('-i', '--ACL', dest='aclip', type='string',metavar='ACLIP', default=False, help=u'접근 통제 IP')
...
class Logger(object):
    ...
    def open_log(self, filename):
        ...
        LOG_START_TIME = strftime("%Y-%m-%d %H:%M:%S", localtime())
    ...
    def log(self, who, what):
        ...
        if self.logfile is not None:
            # write to a file
            #self.logfile.write("%0.10f\t%s\t0x%s\t#%s\n"% (timestamp, who, what, plain))
            if who =='server':
                if os.stat(LOGFILE).st_size > 22428800:#22428800: #104857600
                    splite_logfile = LOGFILE+"_"+str(self.splite_count)
                    os.rename(LOGFILE, splite_logfile)
                    #open(LOGFILE, 'a').close()

                    
                    
                    tmptime = LOG_START_TIME
                    self.open_log(LOGFILE)
                    self.splite_count +=1
                    
                    Process(target=split_logfile_save, args=(splite_logfile, tmptime)).start()
                    #LOG_START_TIME = strftime("%Y-%m-%d %H:%M:%S", localtime())
                    #threading.
                    # t = threading.Thread(target=split_logfile_save,args=(splite_logfile, ))
                
                ttmp = strftime("%M", localtime())
                if self.min != ttmp:
                    self.min = ttmp
                    self.logfile.write(strftime("\n[==============================%Y-%m-%d %H:%M:%S==============================]\n", localtime()))
                    
                self.logfile.write(what)
            
        else:
            # STDOUT output
            #sys.stdout.write("%0.10f\t%s\t0x%s\t#%s\n"% (timestamp, who, what, plain))
            pass
...
class SSHFactory(factory.SSHFactory):
    ...
    def __init__(self, opts):
        global server_id, CSN, EQ_NAME, EQ_IP, LOGFILE, tmp_auth, authkey
        '''
        ...
        # Defaut attributes.
        self.log = Logger()
        LOGFILE = pathlog+'/'+opts.logfile
        
        if opts.logfile is not None:
            self.log.open_log(LOGFILE)
            
        self.spub = opts.serverpubkey
        self.spriv = opts.serverprivkey
        self.sshdebug = sshdebug.SSHDebug(opts.showpassword)
        self.set_block_cmd(opts.CMD_UID)
        server_id = opts.server_id
        CSN =opts.CSN
        EQ_NAME = opts.EQ_NAME
        EQ_IP = opts.host
        tmp_auth = opts.tmp_auth
    ...
    def set_block_cmd(self, CMD_UID):
        global CMD_BLOCK, log_cmd
        ...
        if CMD_UID == None:
            ...
            sql_tmp = "SELECT CMD, ACTION_TYPE FROM KTWIZSTICK.TTP_CMD_SET_ACTION where CMD_UID = %s;"%CMD_UID
   
            tmp =  customDB.mysql_conn(sql_tmp)
...
class SSHServerTransport(transport.SSHServerTransport):
    ...
    def __init__(self):
        ...
        self.digest = ''
        #self._gotdata = True
        #self.connection_flage = True
    
    
    def dataReceived(self, data):
        #self.timeout.reset(30)#
        #self._gotdata = True
        """
        ...
        """
        if self.hmac_auth == True:
            #print '1', [data]
            #print '2', [self.digest]
            if data == self.digest:
                self.hmac_auth = False  
                self.connectionMade()
                return
            else:
                #return
    ...
    def connectionMade(self):
        global acl_ip, authkey, conn_flage, SESSION_ID
        '''
        ...
        self._gotdata = True
        def check():
            print time.time(), self.factory.origin
            if self._gotdata:
                self._gotdata = False
                reactor.callLater(30, check)
            else:
                terminate("client")
        check()
        '''
        ...
        if self.hmac_auth == True:
            ...
            self.digest = hmac.new(authkey, message).digest()
            SESSION_ID = GEN_SESSION_ID(strftime("%Y-%m-%d %H:%M:%S", localtime())+str(authkey))
            return
        print "!!!"        
        ...
        print self.transport.getPeer().host, self.transport.getPeer().port, self.transport.getPeer().type
        if acl_ip == False:
            pass
        else:
            if acl_ip != self.transport.getPeer().host:
                self.transport.loseConnection()
        conn_flage = True
...
class ProxySSHUserAuthClient(userauth.SSHUserAuthClient):
    ...
    def __init__(self, user, instance):
        ...
        global SESSION_ID
        r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'init', 'Sub_type':'','data':''}))
        
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock.sendto(cjson.encode(('SSH', SESSION_ID)), ('127.0.0.1', 12345))
    ...
    def ssh_USERAUTH_FAILURE(self, packet):
        ...
        if self.lastAuth is not "none":
            #customDB.mysql_log_insert(mypool, (strftime("%Y-%m-%d %H:%M:%S", localtime()), server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, 103, 0, 'Login Auth Failure', 'TEXT'), SESSION_ID)
            
            r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'mysql_log_insert', 'Sub_type':'','data':(strftime("%Y-%m-%d %H:%M:%S", localtime()), server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, 103, 0, 'Login Auth Failure', 'TEXT')}))
    ...
    def ssh_USERAUTH_SUCCESS(self, packet):
        ...
        '''
        self.transport.factory.receive.get().addCallback(self.transport.proxy_data_received)
        ...
        #customDB.mysql_log_insert(mypool, (strftime("%Y-%m-%d %H:%M:%S", localtime()), server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, 101, 0, 'LogIn', 'TEXT',tmp_auth), SESSION_ID)
        r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'mysql_log_insert', 'Sub_type':'','data':(strftime("%Y-%m-%d %H:%M:%S", localtime()), server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, 101, 0, 'LogIn', 'TEXT',tmp_auth)}))
    ...
    def show_password(self, password):
        ...
        global ssh_login_username
        ssh_login_username = self.transport.factory.username
        if self.transport.factory.showpass:
...
class SSHCredentialsChecker(object):
    ...
    credentialInterfaces = (credentials.ISSHPrivateKey, credentials.IUsernamePassword,)
...
class ProxySSHConnection(connection.SSHConnection):
    ...
    def __init__(self):
        ...
        self.block_cmd = CMD_BLOCK

        self.log_cmd = log_cmd
        #self.f =  open('sshlog', 'a')
    ...
    def packetReceived(self, messageNum, packet):
        ...
        #print self.transport.factory.origin, chr(messageNum), packet
        #self.transport.factory.transmit.put(chr(messageNum) + packet)
    
    def ssh_comamnd_filter(self, data):
        result  = ''
        for a in data:
            if a == b'\x08' or a == '\x7f':
                result = result[:-1]
    ...
    def log_channel_communication(self, payload):
        global block, ssh_login_username, server_id, CSN, EQ_NAME, tmp_auth, server_block, set_client_cmd, server_data_recv_flage, SESSION_ID
        '''
        ...
        if ord(payload[0]) == 94:
            ...
            if self.transport.factory.origin == 'client':
                #print 'client', [msg], [channel], [datalen], [data]
                #if data == b'\x08' or data == '\x7f':
                    #if len(self.tmp_cmd) != 0:
                        #self.tmp_cmd = self.tmp_cmd[:-1]
                ...
                if data == b'\x03':
                ...
                elif data == b'\x1b[A' or data == b'\x1b[B':
                    server_data_recv_flage = True
                    ...
                    tmp_cmd_len = len(set_client_cmd)
                    if  tmp_cmd_len == 0:
                        pass
                    else:

                        set_client_cmd = self.ssh_comamnd_filter(set_client_cmd)
                        
                        if set_client_cmd.strip() in self.block_cmd:
                            now_t = strftime("%Y-%m-%d %H:%M:%S", localtime())
                            print '%s [%s] (%s) : block commmad : %s'%(now_t, self.transport.factory.origin, ssh_login_username, set_client_cmd)
                            block = True
                            #customDB.mysql_log_insert(mypool, (now_t, server_id,EQ_NAME, EQ_IP,CSN, '',ssh_login_username, 10, 0, set_client_cmd, 'TEXT'), SESSION_ID)
                            r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'mysql_log_insert', 'Sub_type':'','data':(now_t, server_id,EQ_NAME, EQ_IP,CSN, '',ssh_login_username, 10, 0, set_client_cmd, 'TEXT')}))

                            self.transport.factory.transmit.put('^\x00\x00\x00\x00\x00\x00\x00\x01\x03')
                            set_client_cmd = ''
                            return
                        
                        
                        now_t = strftime("%Y-%m-%d %H:%M:%S", localtime())
                        #print '%s [%s] (%s) : %s'%(now_t, self.transport.factory.origin, ssh_login_username, str([set_client_cmd]))
                        
                        LOG_TYPE = 20
                        if tmp_cmd_len > 1000:
                            set_client_cmd = set_client_cmd[0:999]
                            LOG_TYPE = 21
                        elif set_client_cmd in self.log_cmd:
                             LOG_TYPE = 11
                        #customDB.mysql_log_insert(mypool, (now_t, server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, LOG_TYPE, 0, pybase64.standard_b64encode(set_client_cmd), 'BASE64'), SESSION_ID)#tmp.encode('base64')
                        r.lpush(SESSION_ID, 'FFAFF'+cjson.encode({'type':'mysql_log_insert', 'Sub_type':'','data':(now_t, server_id,EQ_NAME, EQ_IP,CSN,'',ssh_login_username, LOG_TYPE, 0, pybase64.standard_b64encode(set_client_cmd), 'BASE64')}))

                    set_client_cmd = ''
                else:
                    set_client_cmd = set_client_cmd+data
                    set_client_cmd = self.ssh_comamnd_filter(set_client_cmd)
            ...
            elif self.transport.factory.origin == 'server':
                #print 'server', [msg], [channel], [datalen], [data]
                if server_data_recv_flage == True:
                    server_data_recv_flage = False
                    set_client_cmd = data
                    
                '''
                for a in self.block_cmd:
                    if data.find(a) > -1:
                        set_client_cmd = data
                '''
                self.transport.factory.log.log(self.transport.factory.origin, data)

                
                
            #self.factory.username
            #self.transport.factory.log.log(self.transport.factory.origin, data)
        if block == True and self.transport.factory.origin == 'server':
            block = False
            #print 'A'
            #print [payload]
            #print [msg], [channel], [datalen], [data]
            #injectiondata = '[block command]'
            injectiondata = '\x00\x00\x00\x0F[block command]'
            #print [struct.pack('>I',len(injectiondata))]
            #datalen = struct.pack('>I',len(injectiondata))            
            #self.transport.factory.transmit.put(msg+channel+datalen+injectiondata)
            self.transport.factory.transmit.put(msg+channel+injectiondata)
            #self.transport.factory.transmit.put('^\x00\x00\x00\x00\x00\x00\x00\x01A')
            #self.transport.factory.transmit.put('^\x00\x00\x00\x00\x00\x00\x00\x0F[block command]')
            #print 'B'
        else:
            self.transport.factory.transmit.put(payload)
