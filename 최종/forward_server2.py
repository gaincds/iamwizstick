...
def multiprocess_start(authkey, forward_to, send_end, tmp_auth):
    try:
        server = TheServer('0.0.0.0', 0, authkey, forward_to, tmp_auth)
        print 'set port :', server.tmp_port
        send_end.send(server.tmp_port)
        server.main_loop()
        
        #p = multiprocessing.Process( target=server.main_loop )
        #p.daemon = True
        #p.start()
        #return server.tmp_port
    except Exception, e:
        print 'multiprocess_start', e
        #print "Ctrl C - Stopping server"
        os.kill(os.getpid(), 9)

def mitmproxy_ssh_main(H_ip, P_port,  send_end, authkey, PROTOCOL, CMD_UID, server_id, CSN, NAME, tmp_auth, clientaddr_ip):
    try:
        #send_end = None,
        ...
        opts.aclip = clientaddr_ip#'0.0.0.0'
        ...
        ser = reactor.listenTCP(opts.localport, factory, interface='0.0.0.0')
...
def setup(authkey, server_id, CSN, Return_Msg, clientaddr_ip = False, tmp_auth = False) :
    ...
    sshproxy =  multiprocessing.Process(target=mitmproxy_ssh_main, args=(IP,  int(L4PORT), send_end, authkey,  PROTOCOL, CMD_UID, server_id, CSN, NAME, tmp_auth, clientaddr_ip) )
    ...
    #recv_end, send_end = multiprocessing.Pipe(False)
    #p = multiprocessing.Process(target=multiprocess_start, args=(authkey, ('127.0.0.1', int(ssh_porxy_port)), send_end, tmp_auth))
    #p.daemon = True
    #p.start()
    
    return str(ssh_porxy_port)#str(recv_end.recv())
